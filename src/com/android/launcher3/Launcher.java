...
public class Launcher extends Activity
    ...
    public boolean onLongClick(View v) {
        if (!isDraggingEnabled()) return false;  //是否允许拖拽
        if (isWorkspaceLocked()) return false;  //是否锁定
        if (mState != State.WORKSPACE) return false;

        if (v == mAllAppsButton) {     //是否为所有app按钮
            onLongClickAllAppsButton(v);   //处理所有app按钮长按事件，即显示所有App页面
            return true;
        }
		//判断是否为长按空白处
        if (v instanceof Workspace) {
            if (!mWorkspace.isInOverviewMode()) {  //判断是否为缩略图模式
        ...
        // on hotseat items.
        final boolean inHotseat = isHotseatLayout(v);  //是否为热键栏
        if (!mDragController.isDragging()) {  //是否处于拖拽过程中
            if (itemUnderLongClick == null) {//如果itemUnderLongClick为null，就是长按空白处一样的处理
